...
- **No reflexive "it's not X, it's Y".** This negation-then-correction construction is the single loudest AI tell in current writing, and it's all over this site. It covers every variant: "It's not X. It's Y.", "X isn't A, it's B.", "not just X but Y", "not because X, but because Y", "The real question isn't X. It's Y." Default to stating the positive claim on its own: "Money is a flow you manage, not a stock you have" becomes "Money is a flow. Stop treating it like a pile." The construction is only earned when the reader genuinely holds belief X and the sentence exists to correct it, and even then: **at most once in a piece**, and never as the punchline closing a section or the post. If two sections both end on one, you're writing to a template. When you catch one, ask what the sentence is actually claiming and write that claim instead. The comparative framing in manifesto mode ("X over Y", "do not confuse X with Y") is a separate, deliberate device and stays allowed there, but don't pad that mode with "not X but Y" either.
...
4. Re-read your own draft specifically hunting for the banned words/patterns above (and, for insight-mode pieces, the mode-specific never-do list). Cut them.
   - Count the "it's not X, it's Y" constructions explicitly. More than one in a piece, or even one sitting at the end of a section, means rewrite.
