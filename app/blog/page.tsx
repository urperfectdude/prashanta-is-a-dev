          ...
          <Link key={post.slug} href={`/blog/${post.slug}`} className="group block">
            <div className="flex gap-6 md:gap-10 items-start">
              <div className="flex flex-col items-end shrink-0 w-[4.5rem] pt-1">
                <span className="text-4xl font-serif font-bold leading-none">{day}</span>
                <span className="text-xs text-muted-foreground text-right mt-1">{monthYear}</span>
              </div>
              <div className="flex flex-col space-y-2">
                <h2 className="text-xl font-medium text-[#7c71ff] group-hover:text-[#7c71ff]/80 transition-colors">
                  {post.title}
                </h2>
